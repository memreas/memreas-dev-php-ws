...
class CompatibilityTest {
	...
	public function __construct() {
		...
		if ($this->isCli) {
			...
			$this->lines [] = sprintf ( '<style type="text/css">%s %s</style>', 'html {font-family:verdana;} .OK {color: #166116;}', '.FAIL {margin-top: 1em; color: #A52C27;} .WARNING {margin-top: 1em; color:#6B036B;}' );
	...
	public function quote($text) {
		return ! $this->isCli ? "<pre>{$text}</pre>" : implode ( "\n", array_map ( function ($t) {
			return '    ' . $t;
		}, explode ( "\n", $text ) ) );
	}
	public function check($info, $func, $text, $required) {
		...
		if ($this->isCli) {
			...
			$text = $level == 'OK' ? "<span class=\"{$level}\">{$info}</span><br />" : "<div class=\"{$level}\">{$info}: [{$level}]<br /><blockquote>{$text}</blockquote></div>";
		}
		$this->write ( $text );
	}
...
$c->addRequire ( 'Ensuring that the version of PHP is >= 5.3.3', function () {
	return version_compare ( phpversion (), '5.3.3', '>=' );
...
if (extension_loaded ( 'suhosin' )) {
	$c->addRequire ( 'Ensuring that phar files can be run with the suhosin patch', function () {
...
$c->addRecommend ( 'Checking if PHP version is >= 5.4.1', function () {
	return version_compare ( phpversion (), '5.4.1', '>=' );
...
$c->addRecommend ( 'Checking if an opcode cache is installed', function () {
