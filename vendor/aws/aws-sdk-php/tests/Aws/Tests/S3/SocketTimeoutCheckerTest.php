...
class SocketTimeoutCheckerTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function setUp() {
		$this->message = '<?xml version="1.0" encoding="UTF-8"?>' . "\n" . '<Error><Code>RequestTimeout</Code><Message>Your socket connection to the server was not read from or ' . 'written to within the timeout period. Idle connections will be closed.</Message>' . '<RequestId>FOO</RequestId><HostId>Bar</HostId></Error>';
	}
	...
	public function testSetsContentLengthIfLocalFile() {
		$body = $this->getMockBuilder ( 'Guzzle\Http\EntityBody' )->setMethods ( array (
				'setSize' 
		) )->setConstructorArgs ( array (
				fopen ( __FILE__, 'r+' ) 
		) )->getMock ();
		
		$body->expects ( $this->once () )->method ( 'setSize' );
		
		$client = $this->getServiceBuilder ()->get ( 's3' );
		$request = $client->put ( '/foo/bar', array (
