...
class ParallelTransferTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testEnsuresTheFileIsLocalAndSeekable() {
		$transfer = new ParallelTransfer ( $this->getServiceBuilder ()->get ( 's3' ), new TransferState ( $this->getMockUploadId () ), EntityBody::factory ( 'foo' ) );
	}
	...
	public function testEnsuresConcurrencyIsSpecified() {
		$transfer = new ParallelTransfer ( $this->getServiceBuilder ()->get ( 's3' ), new TransferState ( $this->getMockUploadId () ), EntityBody::factory ( fopen ( __FILE__, 'r' ) ) );
	}
}
