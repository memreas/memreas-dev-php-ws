...
class AbstractTransferTest extends \Guzzle\Tests\GuzzleTestCase {
	 ...
	 */
	protected $transfer;
	public function prepareTransfer($useRealClient = false, $contentLength = AbstractTransfer::MIN_PART_SIZE, $partLength = AbstractTransfer::MIN_PART_SIZE) {
		$uploadId = $this->getMockBuilder ( 'Aws\S3\Model\MultipartUpload\UploadId' )->setMethods ( array (
				'toParams' 
		) )->getMock ();
		$uploadId->expects ( $this->any () )->method ( 'toParams' )->will ( $this->returnValue ( array (
				'Bucket' => 'foo',
				'Key' => 'bar',
				'UploadId' => 'baz' 
		) ) );
		
		$body = $this->getMockBuilder ( 'Guzzle\Http\EntityBody' )->disableOriginalConstructor ()->setMethods ( array (
				'getContentLength' 
		) )->getMock ();
		$body->expects ( $this->any () )->method ( 'getContentLength' )->will ( $this->returnValue ( $contentLength ) );
		...
		$state->expects ( $this->any () )->method ( 'getUploadId' )->will ( $this->returnValue ( $uploadId ) );
		$state->expects ( $this->any () )->method ( 'getIterator' )->will ( $this->returnValue ( new \ArrayIterator ( array (
