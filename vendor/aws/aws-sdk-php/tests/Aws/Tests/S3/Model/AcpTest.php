...
class AcpTest extends \Guzzle\Tests\GuzzleTestCase {
	protected function getMockedOwner() {
		$owner = $this->getMockBuilder ( 'Aws\S3\Model\Grantee' )->disableOriginalConstructor ()->getMock ();
		$owner->expects ( $this->any () )->method ( 'isCanonicalUser' )->will ( $this->returnValue ( true ) );
		
		return $owner;
	}
	protected function getMockedGrants($count) {
		$grant = $this->getMockBuilder ( 'Aws\S3\Model\Grant' )->disableOriginalConstructor ()->getMock ();
		$grant->expects ( $this->any () )->method ( 'toArray' )->will ( $this->returnValue ( array () ) );
	...
	public function testCanSetGrantsWithTraversable() {
		...
		for($i = 0; $i < 3; $i ++) {
			$grants->append ( $this->getMockBuilder ( 'Aws\S3\Model\Grant' )->disableOriginalConstructor ()->getMock () );
		}
	...
	public function testCanUpdateCommandHeaders() {
		...
		$grant = $this->getMockBuilder ( 'Aws\S3\Model\Grant' )->disableOriginalConstructor ()->getMock ();
		$g1 = clone $grant;
		$g1->expects ( $this->any () )->method ( 'getParameterArray' )->will ( $this->returnValue ( array (
				'GrantRead' => 'id="user-id-1"' 
		) ) );
		$g2 = clone $grant;
		$g2->expects ( $this->any () )->method ( 'getParameterArray' )->will ( $this->returnValue ( array (
				'GrantRead' => 'id="user-id-2"' 
		) ) );
		$g3 = clone $grant;
		$g3->expects ( $this->any () )->method ( 'getParameterArray' )->will ( $this->returnValue ( array (
				'GrantWrite' => 'id="user-id-3"' 
		) ) );
		
		$s3 = $this->getServiceBuilder ()->get ( 's3' );
		$acp = new Acp ( $this->getMockedOwner (), array (
				$g1,
				$g2,
				$g3 
		) );
