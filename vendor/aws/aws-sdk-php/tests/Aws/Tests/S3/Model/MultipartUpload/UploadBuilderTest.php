...
class UploadBuilderTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testHasChainableSetterMethods() {
		...
		$b->setBucket ( 'foo' )->setKey ( 'bar' )->setConcurrency ( 1 )->setMd5 ( 'abc' )->calculateMd5 ( false )->calculatePartMd5 ( true )->setMinPartSize ( 10000 )->setAcp ( $acp )->setOption ( 'Metadata', array (
				'Foo' => 'Baz' 
		) )->setOption ( 'Test', '123' );
	...
	public function testValidatesThatNonSeekableStreamsCannotBeResumed() {
		...
		UploadBuilder::newInstance ()->setBucket ( 'foo' )->setKey ( 'bar' )->setSource ( $source )->resumeFrom ( 'abc' )->setClient ( $this->getServiceBuilder ()->get ( 's3' ) )->build ();
	}
	public function testAllowsForExplicitStateObject() {
		...
		$uploader = UploadBuilder::newInstance ()->resumeFrom ( $state )->setClient ( $this->getServiceBuilder ()->get ( 's3' ) )->setSource ( EntityBody::factory ( 'test' ) )->build ();
	...
	public function testCanResumeStateByLoadingFromS3() {
		...
		$transfer = UploadBuilder::newInstance ()->setBucket ( 'foo' )->setKey ( 'bar' )->resumeFrom ( 'abc' )->setClient ( $client )->setSource ( EntityBody::factory ( 'test' ) )->build ();
	...
	public function testCanCreateNewStateByInitiatingMultipartUpload() {
		...
		$expires = time () + 1000;
		$transfer = UploadBuilder::newInstance ()->setBucket ( 'foo' )->setKey ( 'bar' )->setClient ( $client )->setSource ( __FILE__ )->setHeaders ( array (
				'Foo' => 'Bar' 
		) )->setOption ( 'Expires', $expires )->setAcp ( $acl )->calculateMd5 ( true )->build ();
