...
class S3ClientTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testExplodesKeys() {
		$this->assertEquals ( array (
				...
				'baz ',
				'bar!' 
		), S3Client::explodeKey ( 'foo/baz /bar!' ) );
		$this->assertEquals ( array (
				'foo',
				'baz ',
				'bar!' 
	...
	public function dataForCanCreateObjectUrlsTest() {
		return array (
				...
				array (
						'+1 hour',
						array (),
						'#^https\://foo\.s3\.amazonaws\.com/bar\?.*AWSAccessKeyId.*Expires.*Signature.*$#' 
				),
				array (
						null,
						array (
				...
				array (
						...
						'https://foo.s3.amazonaws.com/bar?response-content-type=' . urlencode ( 'image/png' ) 
				) 
		);
	}
	...
	public function testUploadsDirectories() {
		...
		$mockBuild = $this->getMockBuilder ( 'Aws\S3\Sync\UploadSync' )->disableOriginalConstructor ()->setMethods ( array (
				'transfer' 
		) )->getMock ();
		
		$mockBuild->expects ( $this->once () )->method ( 'transfer' );
		
		$mock = $this->getMockBuilder ( 'Aws\S3\Sync\UploadSyncBuilder' )->setMethods ( array (
				'uploadFromDirectory',
				'setClient',
				'setBucket',
				'setKeyPrefix',
	...
	public function testDownloadsBuckets() {
		...
		$mockBuild = $this->getMockBuilder ( 'Aws\S3\Sync\DownloadSync' )->disableOriginalConstructor ()->setMethods ( array (
				'transfer' 
		) )->getMock ();
		
		$mockBuild->expects ( $this->once () )->method ( 'transfer' );
		
		$mock = $this->getMockBuilder ( 'Aws\S3\Sync\DownloadSyncBuilder' )->setMethods ( array (
				'setDirectory',
				'setClient',
				'setBucket',
				'setKeyPrefix',
	...
	public function testEnsuresSigV4HasRegion() {
		$sig = $this->getMockBuilder ( 'Aws\S3\S3SignatureV4' )->disableOriginalConstructor ()->getMock ();
	...
	public function testEnsuresSignatureImplementsS3Signature() {
		$sig = $this->getMockBuilder ( 'Aws\Common\Signature\SignatureInterface' )->getMockForAbstractClass ();
		$s3 = S3Client::factory ( array (
				Options::SIGNATURE => $sig 
		) );
		$this->assertSame ( $sig, $s3->getSignature () );
	...
	public function testCanForceS3Signature() {
		$sig = $this->getMockBuilder ( 'Aws\S3\S3Signature' )->disableOriginalConstructor ()->getMock ();
		$s3 = S3Client::factory ( array (
				Options::SIGNATURE => $sig,
