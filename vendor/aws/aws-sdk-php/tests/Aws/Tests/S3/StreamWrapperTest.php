...
class StreamWrapperTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testOpensNonSeekableReadStream() {
		...
		$mock = $this->getMockBuilder ( 'Guzzle\Http\EntityBody' )->setConstructorArgs ( array (
				$stream 
		) )->setMethods ( array (
				'seek' 
		) )->getMock ();
		
		$mock->expects ( $this->any () )->method ( 'seek' )->will ( $this->returnValue ( false ) );
		
		$f = $this->getMockBuilder ( 'Guzzle\Stream\PhpStreamRequestFactory' )->setMethods ( array (
				'createStream' 
		) )->getMock ();
		$f->expects ( $this->once () )->method ( 'createStream' )->will ( $this->returnValue ( $mock ) );
	...
	public function testCanSetDelimiterStreamContext() {
		...
		$dir = 's3://bucket';
		$r = opendir ( $dir, stream_context_create ( array (
				's3' => array (
						'delimiter' => '' 
				) 
		) ) );
