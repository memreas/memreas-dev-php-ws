...
class AbstractSyncTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testConstructorValidatesRequirements() {
		$this->getMockBuilder ( 'Aws\S3\Sync\AbstractSync' )->setConstructorArgs ( array (
				array () 
	...
	public function testSendsParallelCommands() {
		...
		$s = $this->getMockBuilder ( 'Aws\S3\Sync\AbstractSync' )->setConstructorArgs ( array (
				array (
						'client' => '',
						'bucket' => '',
						'iterator' => $ai,
		...
		$called = array ();
		$s->expects ( $this->exactly ( 3 ) )->method ( 'transferFiles' )->will ( $this->returnCallback ( function ($data) use(&$called) {
			$called [] = $data;
		} ) );
		$s->transfer ();
	...
	public function testTransfersActions() {
		...
		$client = $this->getMockBuilder ( 'Guzzle\Service\Client' )->setMethods ( array (
				'execute' 
		) )->getMock ();
		$client->expects ( $this->atLeastOnce () )->method ( 'execute' )->with ( $this->equalTo ( array (
				$command 
		) ) );
		
		$a = $this->getMockBuilder ( 'SplFileinfo' )->disableOriginalConstructor ()->getMock ();
		
		$b = $this->getMockBuilder ( 'SplFileinfo' )->disableOriginalConstructor ()->getMock ();
		...
		$s->expects ( $this->exactly ( 2 ) )->method ( 'createTransferAction' )->will ( $this->returnCallback ( function ($file) use(&$called, $a, $command) {
			if ($file === $a) {
