...
class ChangedFilesIteratorTest extends \Guzzle\Tests\GuzzleTestCase {
	public function testRetrievesAndCachesTargetData() {
		...
		$a = $this->getMockBuilder ( 'SplFileinfo' )->setMethods ( array (
				'getSize',
				'getMTime',
				'__toString' 
		...
		$b = $this->getMockBuilder ( 'SplFileinfo' )->setMethods ( array (
				'getSize',
				'getMTime',
				'__toString' 
		) )->disableOriginalConstructor ()->getMock ();
		...
		$targetIterator = new \ArrayIterator ( array (
				$b,
				$a 
		) );
		...
		$this->assertEquals ( array (
				10,
				$ctime 
		), $ref->invoke ( $changed, 'bar' ) );
		$this->assertEquals ( array (
				11,
				$ctime 
		), $ref->invoke ( $changed, 'foo' ) );
	...
	public function testComparesData($size1, $time1, $returnValue, $present) {
		...
		$i1 = new \ArrayIterator ( array (
				$a 
		) );
		$i2 = new \ArrayIterator ( array () );
		$converter = new KeyConverter ();
		
		$iterator = $this->getMockBuilder ( 'Aws\S3\Sync\ChangedFilesIterator' )->setMethods ( array (
				'getTargetData' 
		...
		$iterator->expects ( $this->once () )->method ( 'getTargetData' )->will ( $this->returnValue ( $returnValue ) );
