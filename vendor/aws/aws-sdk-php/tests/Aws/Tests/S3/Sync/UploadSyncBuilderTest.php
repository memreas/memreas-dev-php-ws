...
class UploadSyncBuilderTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testUploadsFromDirectory() {
		$b = UploadSyncBuilder::getInstance ()->uploadFromDirectory ( __DIR__ )->setBucket ( 'foo' )->setClient ( $this->getPreparedClient () )->build ();
		...
		$filenames = array_filter ( iterator_to_array ( $i ), function ($f) {
			return ( string ) $f;
		} );
		$this->assertContains ( __FILE__, $filenames );
	}
	public function testUploadsFromGlob() {
		$b = UploadSyncBuilder::getInstance ()->uploadFromGlob ( __DIR__ . '/*.php' )->setBucket ( 'foo' )->setClient ( $this->getPreparedClient () )->build ();
		$i = $this->readAttribute ( $b, 'options' )->get ( 'iterator' );
		$this->assertInstanceOf ( 'Aws\S3\Sync\ChangedFilesIterator', $i );
		$this->assertInstanceOf ( '\NoRewindIterator', $i->getInnerIterator () );
		$i = $i->getInnerIterator ()->getInnerIterator ();
	...
	public function testCanSetAclOnPutObject() {
		...
		$b = UploadSyncBuilder::getInstance ()->setBaseDir ( __DIR__ )->setSourceIterator ( $i )->setBucket ( 'foo' )->setClient ( $client )->enableDebugOutput ( $out )->setAcl ( 'public-read' )->build ();
		$b->transfer ();
	...
	public function testCanSetAcpOnMultipartUploadsAndEmitsDebug() {
		...
		UploadSyncBuilder::getInstance ()->setBucket ( 'foo' )->setClient ( $client )->setBaseDir ( __DIR__ )->enableDebugOutput ( $out )->setSourceIterator ( new \ArrayIterator ( array (
				new \SplFileInfo ( __FILE__ ) 
		) ) )->setOperationParams ( array (
				'Metadata' => array (
						'foo' => 'bar' 
				) 
		) )->setMultipartUploadSize ( filesize ( __FILE__ ) - 1 )->setAcp ( AcpBuilder::newInstance ()->setOwner ( '123' )->addGrantForEmail ( 'READ_ACP', 'foo@baz.com' )->build () )->build ()->transfer ();
