...
class AbstractSyncBuilderTest extends \Guzzle\Tests\GuzzleTestCase {
	public function testHasSetters() {
		 ...
		 */
		$b->enableDebugOutput ( true )->force ( true )->setBaseDir ( '/base' )->setBucket ( 'bucket' )->setClient ( $this->getServiceBuilder ()->get ( 's3' ) )->setConcurrency ( 2 )->setDelimiter ( '\\' )->setKeyPrefix ( '/prefix/' )->setOperationParams ( array (
				'foo' => 'bar' 
		) )->setSourceFilenameConverter ( $c1 )->setTargetFilenameConverter ( $c1 )->setSourceIterator ( $i )->addRegexFilter ( '/foo/' );
	...
	public function testBuildsUsingSubClasses() {
		...
		$b = $this->getMockBuilder ( 'Aws\S3\Sync\AbstractSyncBuilder' )->setMethods ( array (
				'specificBuild',
				'getTargetIterator' 
		) )->getMockForAbstractClass ();
		$b->expects ( $this->once () )->method ( 'specificBuild' )->will ( $this->returnValue ( $mockSync ) );
		$b->expects ( $this->once () )->method ( 'getTargetIterator' )->will ( $this->returnValue ( $a ) );
		$b->setClient ( $this->getServiceBuilder ()->get ( 's3' ) )->setSourceIterator ( $a )->enableDebugOutput ()->setSourceFilenameConverter ( new KeyConverter () )->setTargetFilenameConverter ( new KeyConverter () )->setBucket ( 'foo' );
