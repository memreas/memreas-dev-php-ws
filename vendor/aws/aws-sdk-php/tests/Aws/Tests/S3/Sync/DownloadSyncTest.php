...
class DownloadSyncTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testCreatesSimpleCommand() {
		$converter = $this->getMockBuilder ( 'Aws\S3\Sync\KeyConverter' )->setMethods ( array (
				'convert' 
		) )->getMock ();
		
		$converter->expects ( $this->once () )->method ( 'convert' )->with ( 's3://foo/baz' )->will ( $this->returnValue ( '/foo/bar' ) );
		
		$sync = $this->getMockBuilder ( 'Aws\S3\Sync\DownloadSync' )->setConstructorArgs ( array (
				array (
		...
		$sync->expects ( $this->once () )->method ( 'createDirectory' )->with ( '/foo/bar' );
	...
	public function testDoesNotAddLeadingSlash($base, $prefix, $path, $result) {
		...
		$sync->expects ( $this->any () )->method ( 'createDirectory' )->will ( $this->returnCallback ( function () {
	...
	public function testCreatesResumableDownloadWhenFileExists() {
		...
		$converter->expects ( $this->once () )->method ( 'convert' )->with ( 's3://foo/baz' )->will ( $this->returnValue ( __FILE__ ) );
		...
		$sync->expects ( $this->once () )->method ( 'createDirectory' )->with ( __FILE__ );
	...
	public function testDoesNotTransferFilesInBatchWithSameNameAsDir() {
		...
		$actualCommands = array (
				$client->getCommand ( 'GetObject', array (
						'Bucket' => 'foo',
						'Key' => 'bar',
						'SaveAs' => '/tmp/test' 
				) ),
				...
				$client->getCommand ( 'GetObject', array (
						'Bucket' => 'foo',
						'Key' => 'bar',
						'SaveAs' => '/tmp/foo/bar.jpg' 
				) ),
				$client->getCommand ( 'GetObject', array (
						'Bucket' => 'foo',
						'Key' => 'bar',
						'SaveAs' => '/tmp/foo/baz/bam' 
				) ),
				$client->getCommand ( 'GetObject', array (
						'Bucket' => 'foo',
						'Key' => 'bar',
						'SaveAs' => '/tmp/foo/baz/bam/a' 
				) ) 
		);
		
		$sync = $this->getMockBuilder ( 'Aws\S3\Sync\DownloadSync' )->disableOriginalConstructor ()->getMock ();
