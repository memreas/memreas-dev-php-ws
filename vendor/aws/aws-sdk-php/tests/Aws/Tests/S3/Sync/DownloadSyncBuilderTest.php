...
class DownloadSyncBuilderTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testCanBuild() {
		...
		$b->setClient ( $client )->setDirectory ( __DIR__ )->setBucket ( 'foo' )->allowResumableDownloads ( true )->setOperationParams ( array (
				'Foo' => 'Bar' 
		) )->build ();
	}
	...
	protected function getDebugSync() {
		...
		$sync = DownloadSyncBuilder::getInstance ()->enableDebugOutput ( $out )->setClient ( $this->getServiceBuilder ()->get ( 's3', true ) )->setBucket ( 'Foo' )->setDirectory ( __DIR__ )->setSourceIterator ( new \ArrayIterator ( array (
				__FILE__ 
		) ) )->build ();
	...
	public function testAddsDebugListenerForCommand() {
		...
		$sync->dispatch ( DownloadSync::BEFORE_TRANSFER, array (
				'command' => $command 
		) );
		rewind ( $out );
