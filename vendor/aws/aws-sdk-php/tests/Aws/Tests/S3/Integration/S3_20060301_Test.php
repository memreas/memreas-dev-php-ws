...
class S3_20060301_Test extends \Aws\Tests\IntegrationTestCase {
	...
	public static function setUpBeforeClass() {
		...
		$client->waitUntilBucketNotExists ( array (
				'Bucket' => $bucket 
		) );
		sleep ( 5 );
	...
	public function testPutObject() {
		...
		// @end
		$this->assertContains ( 'https://' . $this->bucket . '.s3-us-west-2.amazonaws.com/data.txt', $this->getActualOutput () );
	}
	...
	public function testPutObjectFromFile() {
		...
		$this->assertEquals ( file_get_contents ( __FILE__ ), ( string ) $client->getObject ( array (
				'Bucket' => $this->bucket,
				'Key' => 'data_from_file.txt' 
		) )->get ( 'Body' ) );
	}
