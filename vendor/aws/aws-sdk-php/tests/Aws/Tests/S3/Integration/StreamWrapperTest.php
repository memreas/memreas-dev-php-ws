...
class StreamWrapperTest extends \Aws\Tests\IntegrationTestCase {
	...
	public function testUploadsDir() {
		...
		$client = self::getServiceBuilder ()->get ( 's3', true );
		$client->uploadDirectory ( dirname ( __DIR__ ), $this->bucket, 'foo', array (
				'debug' => true 
		) );
		sleep ( 5 );
		$path = 's3://' . $this->bucket;
	...
	public function testUploadsRelativeDir() {
		...
		$client = self::getServiceBuilder ()->get ( 's3', true );
		$client->uploadDirectory ( '../Exception', $this->bucket, 'rel-foo', array (
				'debug' => true 
		) );
	...
	public function testCanDownloadByPrefix() {
		...
		$client = self::getServiceBuilder ()->get ( 's3', true );
		$client->downloadBucket ( '/tmp/swtest', $this->bucket, 'foo', array (
				'debug' => true 
		) );
	...
	public function testCanDownloadToRelativeDir() {
		...
		self::log ( 'Downloading test directory under a prefix' );
		$client = self::getServiceBuilder ()->get ( 's3', true );
		$client->downloadBucket ( '../streamtest', $this->bucket, 'foo/Exception/', array (
				'debug' => true 
		) );
	...
	private function getS3Files($prefix) {
		...
		}, iterator_to_array ( new \RecursiveIteratorIterator ( new \RecursiveDirectoryIterator ( $path, \RecursiveDirectoryIterator::SKIP_DOTS ) ) ) ) );
	}
	private function getTestFiles($dir) {
		return array_values ( array_map ( function ($f) use($dir) {
			return str_replace ( $dir, '', $f );
		}, iterator_to_array ( new \RecursiveIteratorIterator ( new \RecursiveDirectoryIterator ( $dir, \RecursiveDirectoryIterator::SKIP_DOTS ) ) ) ) );
	}
