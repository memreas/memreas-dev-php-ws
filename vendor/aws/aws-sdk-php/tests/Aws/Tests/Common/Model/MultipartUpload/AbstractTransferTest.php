...
class AbstractTransferTest extends \Guzzle\Tests\GuzzleTestCase {
	protected function getMockedTransfer(\Closure $closure = null) {
		$state = $this->getMockBuilder ( 'Aws\Common\Model\MultipartUpload\AbstractTransferState' )->disableOriginalConstructor ()->getMockForAbstractClass ();
		$transfer = $this->getMockBuilder ( 'Aws\Common\Model\MultipartUpload\AbstractTransfer' )->setConstructorArgs ( array (
				$this->getMockBuilder ( 'Aws\Common\Client\AbstractClient' )->disableOriginalConstructor ()->getMockForAbstractClass (),
				$state,
				EntityBody::factory (),
				array (
						'foo' => 'bar' 
	...
	public function testCanAbortUpload() {
		...
		$transfer->expects ( $this->any () )->method ( 'getAbortCommand' )->will ( $this->returnValue ( $command ) );
	...
	public function testWrapsExceptionsThrownDuringUpload() {
		$transfer = $this->getMockedTransfer ();
		$e = new \Exception ( 'foo' );
		$transfer->expects ( $this->once () )->method ( 'transfer' )->will ( $this->throwException ( $e ) );
	...
	public function testCompletesUploadAndDispatchesEvents() {
		$transfer = $this->getMockedTransfer ();
		$model = $this->getMockBuilder ( 'Guzzle\Service\Resource\Model' )->disableOriginalConstructor ()->getMock ();
		$transfer->expects ( $this->once () )->method ( 'complete' )->will ( $this->returnValue ( $model ) );
	...
	public function testIsInvokable() {
		$transfer = $this->getMockedTransfer ();
		$transfer->expects ( $this->once () )->method ( 'transfer' );
		$transfer ();
	}
}
