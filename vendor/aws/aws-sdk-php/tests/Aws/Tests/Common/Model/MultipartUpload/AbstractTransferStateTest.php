...
class AbstractTransferStateTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function setUp() {
		$this->mockUploadId = $this->getMockBuilder ( 'Aws\Common\Model\MultipartUpload\AbstractUploadId' )->setMethods ( array (
				'toParams' 
		) )->getMock ();
		$this->mockUploadId->expects ( $this->any () )->method ( 'toParams' )->will ( $this->returnValue ( array (
				'accountId' => '-',
				'vaultName' => 'foo',
				'uploadId' => 'bar' 
		) ) );
	...
	public function testConstructorInitializesState() {
		$state = $this->getMockForAbstractClass ( 'Aws\Common\Model\MultipartUpload\AbstractTransferState', array (
				$this->mockUploadId 
		) );
	...
	public function testSerializationWorks() {
		$state1 = $this->getMockForAbstractClass ( 'Aws\Common\Model\MultipartUpload\AbstractTransferState', array (
				$this->mockUploadId 
		) );
