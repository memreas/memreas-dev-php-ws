...
class ClientBuilderTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testBuild() {
		$client = ClientBuilder::factory ( 'Aws\\DynamoDb' )->setConfig ( array () )->setConfigDefaults ( array (
				'scheme' => 'https',
				'region' => 'us-east-1',
				'service' => 'dynamodb',
				'service.description' => $this->dynamoDbDescription 
		) )->setConfigRequirements ( array (
				'scheme' 
		) )->setExceptionParser ( new JsonQueryExceptionParser () )->setIteratorsConfig ( array (
				'token_param' => 'foo' 
		) )->build ();
	...
	public function testUsesGlobalEndpoint() {
		$client = ClientBuilder::factory ( 'Aws\\Sts' )->setConfig ( array () )->setConfigDefaults ( array (
				'service' => 'sts',
	...
	public function testBuildAlternate() {
		$client = ClientBuilder::factory ( 'Aws\\DynamoDb' )->setConfigDefaults ( array (
				'scheme' => 'https',
				'region' => 'us-west-1',
				'service' => 'dynamodb',
				'service.description' => $this->dynamoDbDescription,
	...
	public function testEnsuresDescriptionsContainRegions() {
		ClientBuilder::factory ( 'Aws\\DynamoDb' )->setConfig ( array (
	...
	public function testCanCreateSignaturesBasedOnSignatureStringIdentifier($str, $type) {
		try {
			$client = ClientBuilder::factory ()->setConfig ( array (
					'service' => 'foo',
					'region' => 'us-east-1',
					'signature' => $str,
					'service.description' => array (
	...
	public function testAddsDefaultCredentials() {
		$creds = Credentials::factory ( array (
				'key' => 'foo',
				'secret' => 'bar' 
		) );
		$config = array (
