...
class AbstractClientTest extends \Guzzle\Tests\GuzzleTestCase {
	public function testConstructorConfiguresClient() {
		...
		$client = $this->getMockBuilder ( 'Aws\Common\Client\AbstractClient' )->setConstructorArgs ( array (
				$credentials,
				$signature,
				$config 
	...
	public function testAllowsWaiterFactoryInjection() {
		...
		$waiter = $this->getMockBuilder ( 'Aws\Common\Waiter\ResourceWaiterInterface' )->setMethods ( array (
				'wait',
				'setResource',
				'setConfig',
				'setClient' 
		...
		$waiter->expects ( $this->once () )->method ( 'wait' )->will ( $this->returnValue ( $client ) );
		
		$waiter->expects ( $this->once () )->method ( 'setConfig' )->will ( $this->returnValue ( $waiter ) );
		
		$waiter->expects ( $this->once () )->method ( 'setClient' )->will ( $this->returnValue ( $waiter ) );
		
		$factory = $this->getMockBuilder ( 'Aws\Common\Waiter\WaiterFactoryInterface' )->setMethods ( array (
				'build',
				'canBuild' 
		) )->getMock ();
		
		$factory->expects ( $this->once () )->method ( 'build' )->will ( $this->returnValue ( $waiter ) );
		
		$client->setWaiterFactory ( $factory );
		$this->assertSame ( $factory, $client->getWaiterFactory () );
		
		$this->assertSame ( $client, $client->waitUntil ( 'foo', array (
				'baz' => 'bar' 
		) ) );
	}
	public function testClientUpperCasesMagicMethodCallsToCommands() {
		...
		$factory = $this->getMockBuilder ( 'Guzzle\Service\Command\Factory\FactoryInterface' )->getMock ();
		
		$factory->expects ( $this->once () )->method ( 'factory' )->with ( 'FooBar' )->will ( $this->returnValue ( null ) );
	...
	public function testAllowsMagicWaiters() {
		...
		$client->expects ( $this->once () )->method ( 'waitUntil' )->with ( 'Foo', array (
				'baz' => 'bar' 
		) );
		$client->waitUntilFoo ( array (
				'baz' => 'bar' 
		) );
	}
	public function testAllowsMagicIterators() {
		...
		$client->expects ( $this->once () )->method ( 'getIterator' )->with ( 'Foo', array (
				'baz' => 'bar' 
		) );
		$client->getFooIterator ( array (
				'baz' => 'bar' 
		) );
	}
