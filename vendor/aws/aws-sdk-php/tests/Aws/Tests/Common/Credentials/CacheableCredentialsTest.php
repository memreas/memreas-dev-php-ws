...
class CacheableCredentialsTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testSavesCredentialsToCache() {
		...
		$mock = $this->getMockBuilder ( 'Aws\\Common\\Credentials\\Credentials' )->setConstructorArgs ( array (
				'foo',
				'baz',
				'bar',
				1 
		...
		$mock->expects ( $this->any () )->method ( 'isExpired' )->will ( $this->returnCallback ( $callback ) );
