...
class AwsResourceIteratorTest extends \Guzzle\Tests\GuzzleTestCase {
	 ...
	 */
	public function testCanDetermineNextTokenCorrectly($moreKey, $tokenKey, $nextToken, array $data) {
	...
	public function testSendRequest() {
		$model1 = new Model ( array (
				'NextToken' => 'token' 
		) );
		$model2 = new Model ( array () );
		$command = $this->getMockedCommand ();
		$command->expects ( $this->any () )->method ( 'getResult' )->will ( $this->onConsecutiveCalls ( $model1, $model2 ) );
	...
	protected function getMockedCommand() {
		$command = $this->getMockBuilder ( 'Guzzle\Service\Command\OperationCommand' )->disableOriginalConstructor ()->setMethods ( array (
				'getResult',
				'getName',
				'__clone' 
		) )->getMock ();
		...
		$command->expects ( $this->any () )->method ( '__clone' )->will ( $this->returnSelf () );
