...
class AwsResourceIteratorFactoryTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function getDataForBuildTest() {
		...
		$iterator = $this->getMockBuilder ( 'Aws\Common\Iterator\AwsResourceIterator' )->disableOriginalConstructor ()->getMock ();
		...
		$primaryFactory->expects ( $this->any () )->method ( 'build' )->will ( $this->returnValue ( $iterator ) );
		$primaryFactory->expects ( $this->any () )->method ( 'canBuild' )->will ( $this->returnValue ( true ) );
	...
	public function testBuildCreatesIterator($command, array $operations, $otherFactory, $successExpected) {
		...
		try {
			$factory = new AwsResourceIteratorFactory ( array (
