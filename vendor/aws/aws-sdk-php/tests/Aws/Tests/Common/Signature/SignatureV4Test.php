...
class SignatureV4Test extends \Guzzle\Tests\GuzzleTestCase {
	...
	private function getSignature() {
		...
		$signature = $this->getMock ( 'Aws\Common\Signature\SignatureV4', array (
				'getTimestamp',
				'getDateTime' 
		) );
		
		// Hack the shared timestamp
		$signature->expects ( $this->any () )->method ( 'getTimestamp' )->will ( $this->returnValue ( strtotime ( self::DEFAULT_DATETIME ) ) );
		
		// Hack the date time to deal with the wrong date in the example files
		$signature->expects ( $this->any () )->method ( 'getDateTime' )->will ( $this->returnValueMap ( array (
				array (
						DateFormat::RFC1123,
	...
	public function testSuiteProvider() {
		...
		$files = array_filter ( $files, function ($file) {
			return ((strpos ( $file, 'get-header-key-duplicate.' ) === false) && (strpos ( $file, 'get-header-value-order.' ) === false));
		} );
		sort ( $files );
	...
	public function testCreatesCanonicalizedQueryString($headers, $string) {
		...
		$signature = $this->getMockBuilder ( 'Aws\Common\Signature\SignatureV4' )->getMockForAbstractClass ();
