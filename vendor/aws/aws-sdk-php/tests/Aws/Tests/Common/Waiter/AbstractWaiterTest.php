...
class AbstractWaiterTest extends \Guzzle\Tests\GuzzleTestCase {
	public function testIsConfigurable() {
		$waiter = $this->getMockBuilder ( 'Aws\Common\Waiter\AbstractWaiter' )->getMockForAbstractClass ();
	...
	public function testPerformsWait() {
		$waiter = $this->getMockBuilder ( 'Aws\Common\Waiter\AbstractWaiter' )->setMethods ( array (
				'doWait' 
		) )->getMockForAbstractClass ();
		
		$count = 0;
		$waiter->expects ( $this->exactly ( 2 ) )->method ( 'doWait' )->will ( $this->returnCallback ( function () use(&$count) {
			return ++ $count == 2;
		} ) );
	...
	public function testPerformsWaitUntilMaxAttempts() {
		...
		$waiter->expects ( $this->exactly ( 1 ) )->method ( 'doWait' )->will ( $this->returnValue ( false ) );
	...
	public function testPerformsWaitWithEvents() {
		...
		$count = 0;
		$waiter->expects ( $this->exactly ( 3 ) )->method ( 'doWait' )->will ( $this->returnCallback ( function () use(&$count, &$result) {
			$result .= 'b';
			return ++ $count == 3;
		} ) );
	...
	public function testWaiterEventConfigSettings() {
		...
		$waiter->setConfig ( array (
				'waiter.before_attempt' => function () use(&$beforeAttemptCalled) {
					$beforeAttemptCalled ++;
				},
				'waiter.before_wait' => function () use(&$beforeWaitCalled) {
					$beforeWaitCalled ++;
				} 
		) );
		
		$iterations = 0;
		$waiter->expects ( $this->any () )->method ( 'doWait' )->will ( $this->returnCallback ( function () use(&$iterations) {
			return ++ $iterations == 3;
		} ) );
