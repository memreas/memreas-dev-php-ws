...
class ExceptionListenerTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testThrowsServiceSpecificExceptions() {
		...
		$factory = $this->getMockBuilder ( 'Aws\Common\Exception\ExceptionFactoryInterface' )->setMethods ( array (
				'fromResponse' 
		) )->getMock ();
		
		$factory->expects ( $this->once () )->method ( 'fromResponse' )->with ( $request, $response )->will ( $this->returnValue ( $e ) );
