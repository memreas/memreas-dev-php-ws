...
class PerformanceTest extends \Aws\Tests\IntegrationTestCase {
	...
	public function testIssuesMultipleOperationsSerially($totalRequests, $command) {
		...
		$this->log ( "Executed {$totalRequests} " . $command->getName () . " requests in {$elapsed} seconds from a " . self::READ_CAPACITY . " table with {$retries} retries ({$perRequest}/request, {$speed}/second)" );
	}
}
