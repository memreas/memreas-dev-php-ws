...
class WriteRequestBatchTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function getMockedClient() {
		return $this->getMockBuilder ( 'Aws\DynamoDb\DynamoDbClient' )->disableOriginalConstructor ()->getMock ();
	}
	...
	protected function getWriteRequestBatchWithMockedBatch($will) {
		...
		$batch->expects ( $this->any () )->method ( 'isEmpty' )->will ( $this->onConsecutiveCalls ( false, true ) );
		$batch->expects ( $this->any () )->method ( 'flush' )->will ( $will );
	...
	public function getAddItemData() {
		...
		$data [] = array (
				$client->getCommand ( 'DeleteItem' )->set ( 'TableName', 'foo' )->set ( 'Key', array (
		...
		$data [] = array (
				$client->getCommand ( 'PutItem' )->set ( 'TableName', 'foo' )->set ( 'Item', array (
						'id' => array (
								'S' => 'bar' 
						) 
				) ),
		...
		$data [] = array (
				$this->getMock ( 'Aws\DynamoDb\Model\BatchRequest\WriteRequestInterface' ),
				false 
		);
	...
	public function testFlushUnprocessedItems() {
		...
		$item = new UnprocessedRequest ( array (
				'foo' 
		), 'foo' );
		$exceptionUnprocessed = new UnprocessedWriteRequestsException ();
		$exceptionUnprocessed->addItem ( $item );
		$exceptionBatchTransfer = new BatchTransferException ( array (
				$item 
		), array (), $exceptionUnprocessed, $this->getMock ( 'Guzzle\Batch\BatchTransferInterface' ), $this->getMock ( 'Guzzle\Batch\BatchDivisorInterface' ) );
	...
	public function testFlushRandomExceptionFails() {
		$exceptionBatchTransfer = new \Guzzle\Batch\Exception\BatchTransferException ( array (
				$this->getMock ( 'Aws\DynamoDb\Model\BatchRequest\WriteRequestInterface' ) 
		), array (), $this->getMock ( '\Exception' ), $this->getMock ( 'Guzzle\Batch\BatchTransferInterface' ), $this->getMock ( 'Guzzle\Batch\BatchDivisorInterface' ) );
