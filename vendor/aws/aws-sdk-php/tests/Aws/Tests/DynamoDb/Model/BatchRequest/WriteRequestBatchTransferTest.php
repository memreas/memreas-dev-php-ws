...
class WriteRequestBatchTransferTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testTransfersBatches($commandResult, $executeResult, $expectedMessage) {
		...
		$requests = array (
				$this->getMockBuilder ( 'Aws\DynamoDb\Model\BatchRequest\DeleteRequest' )->disableOriginalConstructor ()->getMock () 
		);
		$requests [0]->expects ( $this->any () )->method ( 'getTableName' )->will ( $this->returnValue ( 'foo' ) );
		$requests [0]->expects ( $this->any () )->method ( 'toArray' )->will ( $this->returnValue ( array (
				'foo' 
		) ) );
		
		// Prep the mock command execution and results
		$command = $this->getMock ( 'Aws\Common\Command\JsonCommand', array (), array (
				array () 
		) );
		$command->expects ( $this->any () )->method ( 'set' )->will ( $this->returnValue ( $command ) );
		$command->expects ( $this->any () )->method ( 'isExecuted' )->will ( $this->returnValue ( true ) );
		$command->expects ( $this->any () )->method ( 'getResult' )->will ( $this->returnValue ( $commandResult ) );
		
		// Add the mocked command into the mock client
		$client = $this->getMockedClient ();
		$client->expects ( $this->any () )->method ( 'getCommand' )->will ( $this->returnValue ( $command ) );
		$client->expects ( $this->any () )->method ( 'execute' )->will ( $executeResult ?  : $this->returnValue ( array (
				$command 
		) ) );
