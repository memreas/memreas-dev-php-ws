...
class SessionHandlerTest extends AbstractSessionTestCase {
	...
	public function setUp() {
		...
		$command->expects ( $this->any () )->method ( 'execute' )->will ( $this->returnValue ( array (
				'foo' => 'bar' 
		) ) );
		
		$client->expects ( $this->any () )->method ( 'getIterator' )->will ( $this->returnValue ( array () ) );
		
		$strategy->expects ( $this->any () )->method ( 'doRead' )->will ( $this->returnValue ( array (
				'expires' => time () - 5,
				'data' => 'ANYTHING' 
		) ) );
		$strategy->expects ( $this->any () )->method ( 'doWrite' )->will ( $this->returnValue ( true ) );
		$strategy->expects ( $this->any () )->method ( 'doDestroy' )->will ( $this->returnValue ( true ) );
	}
	...
	public function testSessionGarbageCollectionReturnsFalseOnException() {
		$handler = $this->getMockBuilder ( 'Aws\DynamoDb\Session\SessionHandler' )->disableOriginalConstructor ()->setMethods ( array (
				'garbageCollect' 
		) )->getMock ();
		$handler->expects ( $this->any () )->method ( 'garbageCollect' )->will ( $this->throwException ( new \Exception () ) );
