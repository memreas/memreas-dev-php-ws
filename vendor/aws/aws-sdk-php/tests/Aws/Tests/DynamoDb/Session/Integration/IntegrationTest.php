...
class IntegrationTest extends \Aws\Tests\IntegrationTestCase {
	...
	public function testGeneralSessionStorage() {
		...
		$this->assertEquals ( array (
				'foo' => 'bar' 
		), $_SESSION, '[2] The data was not stored in $_SESSION.' );
		$_SESSION = array ();
		...
		$this->assertEquals ( array (
				'foo' => 'bar' 
		), $_SESSION, '[4] The session data was not retrieved.' );
	...
	public function testGarbageCollection() {
        ...
        )// Sleep 3 seconds in between operations
		null );
	...
	protected function simulateSessionCommit(SessionHandler $handler) {
        ...
        $handler// Instead of session_encode
		null->write ( 'example', $data );
		$handler->close ();
	}
	...
	protected static function deleteSessionsTable() {
		...
		try {
			$client->describeTable ( array (
					'TableName' => $table 
			) );
			// Wait until the table is active
            // Wait until the table is active
			nullself::log ( 'Table exists. Waiting until the status is ACTIVE' );
			$client->waitUntil ( 'table_exists', array (
					'TableName' => $table 
			) );
			// Delete the table to clear out its contents
            // Delete the table to clear out its contents
			nullself::log ( 'Deleting the table' );
			$client->deleteTable ( array (
					'TableName' => $table 
			) );
			$client->waitUntil ( 'table_not_exists', array (
					'TableName' => $table 
			) );
