...
abstract class AbstractSessionTestCase extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function getMockedClient() {
		$client = $this->getMockBuilder ( 'Aws\DynamoDb\DynamoDbClient' )->disableOriginalConstructor ()->getMock ();
		
		$description = $this->getMockBuilder ( 'Guzzle\Service\Description\ServiceDescription' )->disableOriginalConstructor ()->getMock ();
		
		$description->expects ( $this->any () )->method ( 'getApiVersion' )->will ( $this->returnValue ( '2012-08-10' ) );
		
		$client->expects ( $this->any () )->method ( 'getDescription' )->will ( $this->returnValue ( $description ) );
	...
	public function getMockedConfig() {
		$config = $this->getMockBuilder ( 'Aws\DynamoDb\Session\SessionHandlerConfig' )->disableOriginalConstructor ()->getMock ();
