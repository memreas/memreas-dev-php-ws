...
class AbstractLockingStrategyTest extends AbstractSessionTestCase {
	...
	public function testDoDestroyWorksCorrectly($commandResult, $expectedItem) {
		...
		$command->expects ( $this->any () )->method ( 'execute' )->will ( $will );
		
		// Test the doRead method
		$strategy = $this->getMockForAbstractClass ( 'Aws\DynamoDb\Session\LockingStrategy\AbstractLockingStrategy', array (
				$client,
				$config 
		) );
	...
	public function testDoWriteWorksCorrectly($commandResult, $expectedItem) {
		...
		$strategy->expects ( $this->any () )->method ( 'getExtraAttributes' )->will ( $this->returnValue ( array () ) );
