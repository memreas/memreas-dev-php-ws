...
class CloudFront_20130512_Test extends \Aws\Tests\IntegrationTestCase {
	...
	public static function setUpBeforeClass() {
		...
		$s3->waitUntil ( 'bucket_exists', array (
				'Bucket' => self::$bucketName 
		) );
		...
		$s3->waitUntil ( 'object_exists', array (
				'Bucket' => self::$bucketName,
				'Key' => 'foo.txt' 
		) );
	}
	public static function tearDownAfterClass() {
		...
		if (self::$originId) {
			...
			try {
				$result = $cf->getCloudFrontOriginAccessIdentity ( array (
						'Id' => self::$originId 
				) );
	...
	public function testCreatesSignedUrls() {
		...
		$client->waitUntil ( 'DistributionDeployed', array (
				'Id' => self::$distributionId 
		) );
