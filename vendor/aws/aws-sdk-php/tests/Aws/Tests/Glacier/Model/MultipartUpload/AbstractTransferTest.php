...
class AbstractTransferTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function prepareTransfer($useRealClient = false) {
		$uploadId = $this->getMockBuilder ( 'Aws\Glacier\Model\MultipartUpload\UploadId' )->setMethods ( array (
				'toParams' 
		) )->getMock ();
		...
		if ($useRealClient) {
			...
			$client = $this->getMockBuilder ( 'Aws\Glacier\GlacierClient' )->disableOriginalConstructor ()->getMock ();
		}
		...
		$state->expects ( $this->any () )->method ( 'getPartGenerator' )->will ( $this->returnValue ( $generator ) );
	...
	public function testCanGetCommandForUploadPart() {
		...
		$command = $this->callProtectedMethod ( $this->transfer, 'getCommandForPart', array (
				$part,
				true 
		) );
