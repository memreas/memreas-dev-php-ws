...
class UploadBuilderTest extends \Guzzle\Tests\GuzzleTestCase {
	...
	public function testHasChainableSetterMethods() {
		...
		$builder->setAccountId ( 'foo' )->setVaultName ( 'bar' )->setPartGenerator ( UploadPartGenerator::factory ( EntityBody::factory ( 'foo' ), 1024 * 1024 ) )->setConcurrency ( 1 )->setPartSize ( 1024 * 1024 )->setArchiveDescription ( 'abc' );
	...
	public function testValidatesThatNonSeekableStreamsCannotBeResumed() {
		$source = $this->getMockBuilder ( 'Guzzle\Http\EntityBody' )->setMethods ( array (
				'isSeekable' 
		) )->disableOriginalConstructor ()->getMock ();
		
		$source->expects ( $this->any () )->method ( 'isSeekable' )->will ( $this->returnValue ( false ) );
		
		UploadBuilder::newInstance ()->setVaultName ( 'foo' )->setSource ( $source )->resumeFrom ( 'abc' )->setClient ( $this->getServiceBuilder ()->get ( 'glacier' ) )->build ();
	}
	public function testCanResumeStateByLoadingFromGlacier() {
		...
		$transfer = UploadBuilder::newInstance ()->setVaultName ( 'foo' )->resumeFrom ( 'abc' )->setClient ( $client )->setSource ( EntityBody::factory ( 'foo' ) )->setPartGenerator ( $generator )->build ();
	...
	public function testResumeThrowsExceptionIfNoPartGenerator() {
		$client = $this->getServiceBuilder ()->get ( 'glacier' );
		$transfer = UploadBuilder::newInstance ()->setVaultName ( 'foo' )->resumeFrom ( 'abc' )->setClient ( $client )->setSource ( EntityBody::factory ( 'foo' ) )->build ();
	}
	public function testCanCreateNewStateByInitiatingMultipartUpload() {
		...
		$transfer = UploadBuilder::newInstance ()->setVaultName ( 'foo' )->setClient ( $client )->setSource ( EntityBody::factory ( 'foo' ) )->setPartGenerator ( $generator )->build ();
	...
	public function testBuildsDifferentUploaderBasedOnConcurrency() {
		$generator = $this->getMockBuilder ( 'Aws\Glacier\Model\MultipartUpload\UploadPartGenerator' )->setMethods ( array (
				'getPartSize' 
		) )->disableOriginalConstructor ()->getMock ();
