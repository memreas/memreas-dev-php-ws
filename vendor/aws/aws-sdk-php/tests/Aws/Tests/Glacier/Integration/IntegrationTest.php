...
class IntegrationTest extends \Aws\Tests\IntegrationTestCase {
	...
	public function testCrudVaults() {
		...
		foreach ( $vaults as $vault ) {
			...
			$this->client->waitUntil ( 'VaultExists', array (
					'vaultName' => $vault,
					'waiter.max_attempts' => 3 
			) );
		}
		$listVaults = $this->client->getIterator ( 'ListVaults', array (
				'limit' => '5' 
		) );
		...
		foreach ( $vaults as $vault ) {
			$this->client->deleteVault ( array (
					'vaultName' => $vault 
			) );
			$this->client->waitUntil ( 'VaultNotExists', array (
					'vaultName' => $vault 
			) );
		}
