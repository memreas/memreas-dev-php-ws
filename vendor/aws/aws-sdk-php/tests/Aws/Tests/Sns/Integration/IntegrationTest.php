...
class IntegrationTest extends \Aws\Tests\IntegrationTestCase {
	...
	public static function tearDownAfterClass() {
		...
		if (self::$topicArn) {
			try {
				self::getServiceBuilder ()->get ( 'sns' )->deleteTopic ( array (
		...
		if (self::$queueUrl) {
			try {
				self::getServiceBuilder ()->get ( 'sqs' )->deleteQueue ( array (
	...
	public function testListsSubscriptions($subscriptionArn) {
		...
		$i = $this->sns->getIterator ( 'ListSubscriptionsByTopic', array (
				'TopicArn' => self::$topicArn 
		) );
	...
	public function testListsTopicAttributes($topicArn) {
		$result = $this->sns->getTopicAttributes ( array (
				'TopicArn' => $topicArn 
		) );
