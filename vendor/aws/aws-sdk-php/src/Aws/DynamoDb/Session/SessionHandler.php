...
/**
 * Provides an interface for using Amazon DynamoDB as a session store by hooking into PHP's session handler hooks.
 * Once
...
class SessionHandler implements SessionHandlerInterface {
	 ...
	 * @param LockingStrategyInterface $strategy
	 *        	Locking strategy for performing session locking logic
	 * @param SessionHandlerConfig $config
	 *        	Configuration options for the session handler
	 */
	public function __construct(DynamoDbClient $client, LockingStrategyInterface $strategy, SessionHandlerConfig $config) {
		$this->client = $client;
	 ...
	 *        	RCUs for table read throughput
	 * @param int $writeCapacityUnits
	 *        	WCUs table write throughput
	 *        	
	...
	public function createSessionsTable($readCapacityUnits, $writeCapacityUnits) {
		...
		$this->client->waitUntil ( 'table_exists', array (
				'TableName' => $tableName 
		) );
	 ...
	 *        	The session ID
	 * @param string $data
	 *        	The serialized session data to write
	 *        	
	 * @return bool Whether or not the operation succeeded
	 */
	public function write($id, $data) {
		// Write the session data using the selected locking strategy
		$this->sessionWritten = $this->lockingStrategy->doWrite ( $this->formatId ( $id ), $data, ($data !== $this->dataRead) );
