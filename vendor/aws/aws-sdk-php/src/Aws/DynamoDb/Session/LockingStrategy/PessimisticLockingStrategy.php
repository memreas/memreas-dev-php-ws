 ...
 * another process is reading/writing to it.
 * Pessimistic locking can be
...
class PessimisticLockingStrategy extends AbstractLockingStrategy {
	/**
	 *
	 * @ERROR!!! Adds the defaults for the pessimistic locking strategy if not set
	 */
	...
	/**
	 *
	 * @ERROR!!! Retries the request until the lock can be acquired
	 */
	public function doRead($id) {
		...
		do {
			try {
				...
				usleep ( rand ( $this->config->get ( 'min_lock_retry_microtime' ), $this->config->get ( 'max_lock_retry_microtime' ) ) );
