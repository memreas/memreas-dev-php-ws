...
class AwsResourceIterator extends ResourceIterator {
	...
	/**
	 *
	 * @ERROR!!! This AWS specific version of the resource iterator provides a default implementation of the typical AWS iterator
		...
		if ($tokenParam = $this->get ( 'token_param' )) {
			...
			if (is_array ( $tokenParam )) {
				if (is_array ( $this->nextToken ) && count ( $tokenParam ) === count ( $this->nextToken )) {
					...
					throw new RuntimeException ( 'The definition of the iterator\'s token parameter and the actual token ' . 'value are not compatible.' );
				}
			} else {
