...
class ConfigResourceWaiter extends AbstractResourceWaiter {
	...
	/**
	 *
	 * @param WaiterConfig $waiterConfig
	...
	protected function doWait() {
		...
		try {
			...
			throw new InvalidArgumentException ( $this->waiterConfig->get ( WaiterConfig::WAITER_NAME ) . ' waiter validation failed:  ' . $e->getMessage (), $e->getCode (), $e );
	 ...
	 *
	 * @return bool null true for success, false for failure, and null for no transition
	 */
	...
	protected function checkResult(Model $result) {
		...
		if ($this->waiterConfig->get ( WaiterConfig::SUCCESS_TYPE ) == 'output' && $this->checkPath ( $result, $this->waiterConfig->get ( WaiterConfig::SUCCESS_PATH ), $this->waiterConfig->get ( WaiterConfig::SUCCESS_VALUE ) )) {
			return true;
		}
		...
		if ($this->waiterConfig->get ( WaiterConfig::FAILURE_TYPE ) == 'output') {
			...
			if ($failureValue) {
				...
				if ($this->checkPath ( $result, $key, $failureValue, false )) {
					...
					$triggered = array_intersect ( ( array ) $this->waiterConfig->get ( WaiterConfig::FAILURE_VALUE ), array_unique ( ( array ) $result->getPath ( $key ) ) );
					// fast fail because the failure case was satisfied
					throw new RuntimeException ( 'A resource entered into an invalid state of "' . implode ( ', ', $triggered ) . '" while waiting with the "' . $this->waiterConfig->get ( WaiterConfig::WAITER_NAME ) . '" waiter.' );
				}
			}
		}
