	...
	public static function factory($config = array()) {
		...
		if (! $config [Options::KEY] || ! $config [Options::SECRET]) {
			...
			if ($envKey && $envSecret) {
				...
				$credentials = new RefreshableInstanceProfileCredentials ( new static ( '', '', '', 1 ), $config [Options::CREDENTIALS_CLIENT] );
			}
		...
		if ($cacheKey && $cache) {
			if ($cache === 'true' || $cache === true) {
				...
				if (! extension_loaded ( 'apc' )) {
					throw new RequiredExtensionNotLoadedException ( 'PHP has not been compiled with APC. Unable to cache ' . 'the credentials.' );
				} elseif (! class_exists ( 'Doctrine\Common\Cache\ApcCache' )) {
					throw new RuntimeException ( 'Cannot set ' . Options::CREDENTIALS_CACHE . ' to true because the Doctrine cache component is ' . 'not installed. Either install doctrine/cache or pass in an instantiated ' . 'Guzzle\Cache\CacheAdapterInterface object' );
				}
