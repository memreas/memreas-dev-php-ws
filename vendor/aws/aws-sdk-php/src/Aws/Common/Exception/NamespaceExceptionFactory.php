 ...
 * namespace that contains exceptions.
 * Exception classes are expected to be in
...
class NamespaceExceptionFactory implements ExceptionFactoryInterface {
	 ...
	 */
	public function __construct(ExceptionParserInterface $parser, $baseNamespace, $defaultException = 'Aws\Common\Exception\ServiceResponseException') {
		$this->parser = $parser;
	...
	public function fromResponse(RequestInterface $request, Response $response) {
		...
		$className = $this->baseNamespace . '\\' . str_replace ( array (
				'AWS.',
				'.' 
		), '', $parts ['code'] );
