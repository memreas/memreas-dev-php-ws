...
class CloudFrontClient extends AbstractClient {
	...
	public static function factory($config = array()) {
		...
		) )->setExceptionParser ( new DefaultXmlExceptionParser () )->setIteratorsConfig ( array (
				'token_param' => 'Marker',
	...
	/**
	 * Create a signed URL.
	 * Keep in mind that URLs meant for use in media/flash players may have different requirements
	...
	public function getSignedUrl(array $options) {
		...
		$url->getQuery ()->set ( 'Signature', $signedPolicy )->set ( 'Key-Pair-Id', $options ['key_pair_id'] );
	 ...
	 *        	File containing the OpenSSL private key
	 *        	
	 * @return string
	 */
	 ...
	 *
	 * @param string $scheme
	 *        	Parsed scheme without wildcards
	 * @param string $url
	 *        	URL that is being signed
	 * @param int $expires
	 *        	Time in which the signature expires
	 *        	
	 * @return string
	...
	protected function createCannedPolicy($scheme, $url, $expires) {
		...
		if ($scheme == 'http' || $scheme == 'https') {
			...
			$resource = ltrim ( $pathParts ['dirname'] . '/' . $pathParts ['basename'], '/' ) . (isset ( $parts ['query'] ) ? "?{$parts['query']}" : '');
		} else {
			throw new InvalidArgumentException ( "Invalid URI scheme: {$scheme}. Must be one of http or rtmp." );
		}
		
		return sprintf ( '{"Statement":[{"Resource":"%s","Condition":{"DateLessThan":{"AWS:EpochTime":%d}}}]}', $resource, $expires );
	}
}
