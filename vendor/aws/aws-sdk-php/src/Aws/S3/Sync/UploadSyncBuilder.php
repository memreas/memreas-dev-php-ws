...
class UploadSyncBuilder extends AbstractSyncBuilder {
	...
	public function uploadFromDirectory($path) {
		$this->baseDir = $path;
		$this->sourceIterator = $this->filterIterator ( new \RecursiveIteratorIterator ( new \RecursiveDirectoryIterator ( $path, FI::SKIP_DOTS | FI::UNIX_PATHS | FI::FOLLOW_SYMLINKS ) ) );
	...
	public function uploadFromGlob($glob) {
		$this->sourceIterator = $this->filterIterator ( new \GlobIterator ( $glob, FI::SKIP_DOTS | FI::UNIX_PATHS | FI::FOLLOW_SYMLINKS ) );
	...
	/**
	 * Set the multipart upload size threshold.
	 * When the size of a file exceeds this value, the file will be uploaded
	...
	protected function addCustomParamListener(HasDispatcherInterface $sync) {
		...
		$params = $this->params;
		$sync->getEventDispatcher ()->addListener ( UploadSync::BEFORE_MULTIPART_BUILD, function (Event $e) use($params) {
