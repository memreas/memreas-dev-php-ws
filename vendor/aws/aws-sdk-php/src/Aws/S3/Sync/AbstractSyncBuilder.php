...
abstract class AbstractSyncBuilder {
	 ...
	 *
	 * @param FileNameConverterInterface $converter
	...
	/**
	 * Set the base directory of the files being transferred.
	 * The base directory is removed from each file path before
	 ...
	 * Enable debug mode
	 *
	 * @param bool|resource $enabledOrResource
	 *        	Set to true or false to enable or disable debug output. Pass an opened
	 ...
	 *
	 * @param string $search
	 *        	Regular expression search (in preg_match format). Any filename that matches this regex
	...
	public function build() {
		...
		if (! $this->forcing) {
			$this->sourceIterator->rewind ();
			$this->sourceIterator = new ChangedFilesIterator ( new \NoRewindIterator ( $this->sourceIterator ), $this->getTargetIterator (), $this->sourceConverter, $this->targetConverter );
			$this->sourceIterator->rewind ();
		}
