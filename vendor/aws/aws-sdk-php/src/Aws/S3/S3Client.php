...
class S3Client extends AbstractClient {
	...
	public static function factory($config = array()) {
		...
		) )->setExceptionParser ( $exceptionParser )->setIteratorsConfig ( array (
		...
		$client->addSubscriber ( new UploadBodyListener ( array (
				'PutObject',
				'UploadPart' 
		) ) );
	...
	private static function createBackoffPlugin(S3ExceptionParser $exceptionParser) {
		return new BackoffPlugin ( new TruncatedBackoffStrategy ( 3, new HttpBackoffStrategy ( null, new SocketTimeoutChecker ( new CurlBackoffStrategy ( null, new ExpiredCredentialsChecker ( $exceptionParser, new ExponentialBackoffStrategy () ) ) ) ) ) );
	}
	...
	private static function createSignature($config) {
		...
		if ($currentValue == 'v4' || (! $currentValue && isset ( $config ['region'] ) && substr ( $config ['region'], 0, 3 ) == 'cn-')) {
		...
		throw new InvalidArgumentException ( 'The provided signature value is ' . 'not an instance of S3SignatureInterface' );
	}
...
! preg_match ( '/^[a-z0-9][a-z0-9\-\.]*[a-z0-9]?$/', $bucket )) {
	 ...
	 *        	Request to generate the URL for. Use the factory methods of the client to
	 ...
	 * If an expiration time is provided, the URL will
	 ...
	 * @param array $args
	 *        	Arguments to the GetObject command. Additionally you can specify a "Scheme" if you would
	...
	public function getObjectUrl($bucket, $key, $expires = null, array $args = array()) {
		$command = $this->getCommand ( 'GetObject', $args + array (
				'Bucket' => $bucket,
				'Key' => $key 
		) );
	 ...
	 * Use the {@see ClearBucket} object directly
	...
	public function doesBucketExist($bucket, $accept403 = true, array $options = array()) {
		return $this->checkExistenceWithCommand ( $this->getCommand ( 'HeadBucket', array_merge ( $options, array (
				'Bucket' => $bucket 
		) ) ), $accept403 );
	}
	...
	public function doesObjectExist($bucket, $key, array $options = array()) {
		return $this->checkExistenceWithCommand ( $this->getCommand ( 'HeadObject', array_merge ( $options, array (
				'Bucket' => $bucket,
				'Key' => $key 
		) ) ) );
	}
	...
	public function doesBucketPolicyExist($bucket, array $options = array()) {
		return $this->checkExistenceWithCommand ( $this->getCommand ( 'GetBucketPolicy', array_merge ( $options, array (
				'Bucket' => $bucket 
		) ) ) );
	}
	 ...
	 * If the upload size exceeds the specified threshold, the upload
	 ...
	 *        	Key of the object
	 * @param mixed $body
	 *        	Object data to upload. Can be a Guzzle\Http\EntityBodyInterface, stream resource, or
	...
	public function deleteMatchingObjects($bucket, $prefix = '', $regex = '', array $options = array()) {
		...
		$clear = new ClearBucket ( $this, $bucket );
		$iterator = $this->getIterator ( 'ListObjects', array (
				'Bucket' => $bucket,
				'Prefix' => $prefix 
		) );
	 ...
	 *        	
	 * @return bool
	 * @throws S3Exception \Exception there is an unhandled exception
	 */
