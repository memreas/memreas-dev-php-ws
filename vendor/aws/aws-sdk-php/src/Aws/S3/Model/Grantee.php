...
class Grantee implements ToArrayInterface {
	...
	public function setId($id, $expectedType = null) {
		...
		if ($expectedType && $expectedType !== $this->type) {
			throw new UnexpectedValueException ( 'The type of the grantee after ' . 'setting the ID did not match the specified, expected type "' . $expectedType . '" but received "' . $this->type . '".' );
		}
	...
	public function setDisplayName($displayName) {
		if ($this->type === GranteeType::USER) {
			...
			if ($displayName) {
				throw new LogicException ( 'The display name can only be set ' . 'for grantees specified by ID.' );
			}
		}
