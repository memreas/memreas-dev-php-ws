...
class StreamWrapper {
	/**
	 *
	 * @var resource null context (this is set by PHP when a context is used)
	 */
	public $context;
	 ...
	 * Works on buckets, keys, and prefixes
	 *
	 * @param string $path        	
	 * @param int $flags        	
	 *
	 ...
	 * @param int $mode
	 *        	Permissions. 700-range permissions map to ACL_PUBLIC. 600-range permissions map to
	 *        	ACL_AUTH_READ. All other permissions map to ACL_PRIVATE. Expects octal form.
	 * @param int $options
	 *        	A bitwise mask of values, such as STREAM_MKDIR_RECURSIVE. (unused)
	 *        	
	 * @return bool
	 ...
	 *
	 * @param string $path
	 *        	The path to the directory (e.g. "s3://dir[</prefix>]")
	...
	/**
	 * Get the bucket and key from the passed path (e.g.
	 * s3://bucket/key)
	 *
	 * @param string $path
	...
	protected function openReadStream(array $params, array &$errors) {
		...
		$this->body = $factory->fromRequest ( $request, array (), array (
