...
abstract class CacheTest extends \Doctrine\Tests\DoctrineTestCase {
	...
	public function testDeleteAllAndNamespaceVersioningBetweenCaches() {
		...
		/*
		 * Both providers are initialized with the same namespace version, so they can see entries set by each other.
		 */
		...
		/*
		 * Deleting all entries through one provider will only increment the namespace version on that object (and in the cache itself, which new instances will use to initialize). The second provider will retain its original version and still see stale data.
		 */
		...
		/*
		 * A new cache provider should not see the deleted entries, since its namespace version will be initialized.
		 */
	...
	public function testFlushAllAndNamespaceVersioningBetweenCaches() {
		...
		/*
		 * Deleting all elements from the first provider should increment its namespace version before saving the first entry.
		 */
		$cache1->deleteAll ();
		$this->assertTrue ( $cache1->save ( 'key1', 1 ) );
		
		/*
		 * The second provider will be initialized with the same namespace version upon its first save operation.
		 */
		$this->assertTrue ( $cache2->save ( 'key2', 2 ) );
		
		/*
		 * Both providers have the same namespace version and can see entires set by each other.
		 */
		$this->assertTrue ( $cache1->contains ( 'key1' ) );
		$this->assertTrue ( $cache1->contains ( 'key2' ) );
		$this->assertTrue ( $cache2->contains ( 'key1' ) );
		...
		/*
		 * Flushing all entries through one cache will remove all entries from the cache but leave their namespace version as-is.
		 */
		...
		/*
		 * Inserting a new entry will use the same, incremented namespace version, and it will be visible to both providers.
		 */
		$this->assertTrue ( $cache1->save ( 'key1', 1 ) );
		$this->assertTrue ( $cache1->contains ( 'key1' ) );
		$this->assertTrue ( $cache2->contains ( 'key1' ) );
		...
		/*
		 * A new cache provider will be initialized with the original namespace version and not share any visibility with the first two providers.
		 */
		$cache3 = $this->_getCacheDriver ();
		$this->assertFalse ( $cache3->contains ( 'key1' ) );
		$this->assertFalse ( $cache3->contains ( 'key2' ) );
	...
	public function testFetchMissShouldReturnFalse() {
		...
		/*
		 * Ensure that caches return boolean false instead of null on a fetch miss to be compatible with ORM integration.
		 */
