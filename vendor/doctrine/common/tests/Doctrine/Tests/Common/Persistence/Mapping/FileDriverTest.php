...
class FileDriverTest extends DoctrineTestCase {
	...
	public function testGetElementFromFile() {
		...
		$locator->expects ( $this->once () )->method ( 'findMappingFile' )->with ( $this->equalTo ( 'stdClass' ) )->will ( $this->returnValue ( __DIR__ . '/_files/stdClass.yml' ) );
	...
	public function testGetAllClassNamesFromMappingFile() {
		$locator = $this->newLocator ();
		$locator->expects ( $this->any () )->method ( 'getAllClassNames' )->with ( $this->equalTo ( null ) )->will ( $this->returnValue ( array (
				'stdClass' 
		) ) );
		$driver = new TestFileDriver ( $locator );
	...
	public function testGetAllClassNamesBothSources() {
		$locator = $this->newLocator ();
		$locator->expects ( $this->any () )->method ( 'getAllClassNames' )->with ( $this->equalTo ( 'global' ) )->will ( $this->returnValue ( array (
				'stdClass' 
		) ) );
		$driver = new TestFileDriver ( $locator );
		$driver->setGlobalBasename ( "global" );
	...
	public function testIsNotTransient() {
		$locator = $this->newLocator ();
		$locator->expects ( $this->once () )->method ( 'fileExists' )->with ( $this->equalTo ( 'stdClass' ) )->will ( $this->returnValue ( true ) );
	...
	public function testIsTransient() {
		$locator = $this->newLocator ();
		$locator->expects ( $this->once () )->method ( 'fileExists' )->with ( $this->equalTo ( 'stdClass2' ) )->will ( $this->returnValue ( false ) );
	...
	private function newLocator() {
		...
		$locator->expects ( $this->any () )->method ( 'getPaths' )->will ( $this->returnValue ( array (
				__DIR__ . "/_files" 
		) ) );
		return $locator;
	}
