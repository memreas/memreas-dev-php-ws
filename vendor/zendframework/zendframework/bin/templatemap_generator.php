...
if (isset ( $opts->v )) {
	if (! is_dir ( $opts->v )) {
		echo 'Invalid view template directory provided' . PHP_EOL . PHP_EOL;
		echo $opts->getUsageMessage ();
		exit ( 2 );
	}
	$viewPath = $opts->v;
...
if (isset ( $opts->o )) {
	...
	if ('-' == $output) {
		...
		echo "Cannot write to '$output'; aborting." . PHP_EOL . PHP_EOL . $opts->getUsageMessage ();
		exit ( 2 );
	} elseif (file_exists ( $output ) && ! $opts->getOption ( 'w' ) && ! $appending) {
		echo "Template map file already exists at '$output'," . PHP_EOL . "but 'overwrite' or 'appending' flag was not specified; aborting." . PHP_EOL . PHP_EOL . $opts->getUsageMessage ();
		exit ( 2 );
	} else {
