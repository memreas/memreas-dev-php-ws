...
class AddExistMediaToEvent {
	...
	public function exec($frmweb = false, $output = '') {
		...
		$query_event->select ( 'e.event_id' )->from ( 'Application\Entity\Event', 'e' )->where ( 'e.event_id=?1' )->setParameter ( 1, $event_id );
		...
		if (empty ( $result )) {
			...
			foreach ( $medias->media_id as $media_id ) {
				...
				$media_query->select ( 'm.media_id' )->from ( 'Application\Entity\Media', 'm' )->where ( 'm.media_id=?1' )->setParameter ( 1, $media_id );
				$result = $media_query->getQuery ()->getResult ();
				if (! empty ( $result )) {
					// Check if media is added or not
					$check_media = $this->dbAdapter->createQueryBuilder ();
					$check_media->select ( 'em' )->from ( 'Application\Entity\EventMedia', 'em' )->where ( "em.event_id='$event_id' AND em.media_id='$media_id'" );
