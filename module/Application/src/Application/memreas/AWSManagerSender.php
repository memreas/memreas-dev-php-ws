...
class AWSManagerSender
{
    ...
    public function snsProcessMediaPublish ($message_data)
    {
        ...
        try {
            ...
            if ((MemreasConstants::MEMREAS_TRANSCODER) &&
                     (MemreasConstants::MEMREAS_TRANSCODE_URL)) {
                /*
                 * Publish to web server here
                 */
                Mlog::addone(
                        'Publishing to MemreasConstants::MEMREAS_TRANSCODE_URL', 
                        MemreasConstants::MEMREAS_TRANSCODE_URL);
                ...
                if (MemreasConstants::MEMREAS_TRANSCODER) {
                    ...
                    Mlog::addone('Just published to MemreasConstants::QUEUEURL', 
                            MemreasConstants::QUEUEURL);
                    Mlog::addone('json', $json);
                }
        } catch (\Exception $e) {
            error_log(
    ...
    public function webserviceUpload ($user_id, $dirPath, $file_name, 
            $content_type)
    {
        $s3_data = array();
