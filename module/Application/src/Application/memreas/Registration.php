				...
				if (! $checkvalidemail) {
					...
					if (! empty ( $result )) {
						...
						if (! empty ( $ndata->from_id ) && ! empty ( $ndata->event_id )) {
							...
							$xml_input = '<xml><addfriendtoevent>' . '<user_id>' . $ndata->from_id . '</user_id>' . '<event_id>' . $ndata->event_id . '</event_id>' . '<friends><friend>' . '<network_name>memreas</network_name>' . '<friend_name>' . $username . '</friend_name>' . '<profile_pic_url><![CDATA[' . $url . ']]>' . '</profile_pic_url> </friend> </friends> </addfriendtoevent></xml>';
