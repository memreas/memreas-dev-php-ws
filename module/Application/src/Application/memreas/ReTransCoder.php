...
class ReTransCoder
{
    ...
    public function exec ()
    {
        try {
            ...
            if (! empty($media_id)) {
                $media = $this->dbAdapter->getRepository(
                        'Application\Entity\Media')->findOneBy(
                        array(
                                'media_id' => $media_id
                        ));
                if ($media) {
                    ...
                    $message_data = array(
                            ...
                            's3path' => $media->user_id . '/' . $media->media_id .
                                    ...
                                    'is_video' => empty(
                                            $meta['S3_files']['is_video']) ? 0 : 1,
                                    'is_audio' => empty(
                                            $meta['S3_files']['is_audio']) ? 0 : 1
                    );
                    Mlog::addone('$message_data', $message_data);
                    Mlog::addone('$meta [S3_files] [is_video] ', 
                            $meta['S3_files']['is_video']);
                } else {
