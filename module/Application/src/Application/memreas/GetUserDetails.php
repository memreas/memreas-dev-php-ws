    ...
    public function exec ($frmweb = false, $output = '')
    {
        try {
            ...
            if (empty($result_user)) {
                ...
                $output .= '<username>' . $result_user[0]->username .
                         '</username>';
                $output .= '<email>' . $result_user[0]->email_address .
                         '</email>';
                $metadata = $result_user[0]->metadata;
                $metadata = json_decode($metadata, true);
                if (isset($metadata['alternate_email']))
                    $output .= '<alternate_email>' .
                             $metadata['alternate_email'] . '</alternate_email>';
                else
                ...
                if (! empty($metadata['subscription'])) {
                    Mlog::addone('if (!empty( $metadata [subscription] )', 
                            $metadata['subscription']);
                    $subscription = $metadata['subscription'];
                    $output .= '<subscription><plan>' . $subscription['plan'] .
                             '</plan><plan_name>' . $subscription['name'] .
                             '</plan_name></subscription>';
                } else {
                    Mlog::addone('if (empty( $metadata [subscription] )', 
                            '<subscription><plan>FREE</plan></subscription>');
                ...
                if ((! empty($data)) && ($data['status'] == 'Success')) {
                    ...
                    $output .= '</account_type>';
                    $output .= "<buyer_balance>" . $data['buyer_balance'] .
                             "</buyer_balance>";
                    $output .= "<seller_balance>" . $data['seller_balance'] .
                             "</seller_balance>";
                } else {
                ...
                $output .= '<profile><![CDATA[' . $profile_image .
                         ']]></profile>';
            }
