	...
	public function exec($frmweb = false, $output = '') {
		...
		if (empty ( $result_groups )) {
			...
			if (! empty ( $group_id ))
				...
				$message = "You have no friend with this network.";
		} else {
			$status = 'Success';
			$output .= '<friends>';
			foreach ( $result_groups as $friend ) {
				...
				if ($friend->network != 'memreas')
					...
					$output .= '<friend_photo>' . urldecode ( $this->url_signer->signArrayOfUrls ( $friend->url_image ) ) . '</friend_photo>';
				$output .= '</friend>';
			}
			$output .= '</friends>';
		}
