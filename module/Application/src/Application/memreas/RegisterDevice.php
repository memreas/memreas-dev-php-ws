	...
	public function exec() {
		...
		if (! empty ( $username ) && ! empty ( $devicetoken ) && ! empty ( $devicetype )) {
			...
			if ($result) {
				...
				$qb = $this->dbAdapter->createQueryBuilder ();
				$q = $qb->update ( '\Application\Entity\Device', 'd' )->set ( 'd.device_token', $qb->expr ()->literal ( $devicetoken ) )->set ( 'd.update_time', $qb->expr ()->literal ( $time ) )->where ( 'd.user_id = ?1 AND d.device_type = ?2' )->setParameter ( 1, $result ['user_id'] )->setParameter ( 2, $devicetype )->getQuery ();
				$p = $q->execute ();
			} else {
