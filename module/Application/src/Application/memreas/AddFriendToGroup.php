	...
	public function exec($frmweb = false, $output = '') {
		...
		if (empty ( $result )) {
			...
			foreach ( $friends->friend as $friend ) {
				...
				$query->select ( 'f.friend_id' )->from ( 'Application\Entity\Friend', 'f' )->where ( 'f.social_username = ?1' )->setParameter ( 1, $friend_name );
				...
				if (empty ( $result )) {
					...
					$friend_id = $result [0] ['friend_id'];
					
					// Check if friend has already added to group before yet
				$query_group = $this->dbAdapter->createQueryBuilder ();
				$query_group->select ( 'fg' )->from ( 'Application\Entity\FriendGroup', 'fg' )->where ( "fg.group_id = '{$group_id}' AND fg.friend_id = '{$friend_id}'" );
