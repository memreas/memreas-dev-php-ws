        ...
        try {
            ...
            if (isset($_POST['xml']) && ! empty($_POST['xml'])) {
                ...
                if (isset($data->addmediaevent->user_id)) {
                    $user_id = addslashes(
                            trim($data->addmediaevent->user_id));
                } else {
                ...
                $email = isset($data->addmediaevent->email) ? addslashes(
                        trim($data->addmediaevent->email)) : '';
            } else {
                // Old code uses POST
                // Fetch user_id
            ...
            if ($is_server_image == 1) {
                ...
                if (strcasecmp($file_type[0], 'image') == 0) {
                    $json_array = array();
                    $s3file = (isset($_POST['s3file_name']) ||
                             isset($s3file_name)) ? $s3path . $s3file_name : $s3url;
