...
class AddMediaEvent
{
            ...
            $service_locator)
    {
        ...
        if (! $this->AddNotification) {
            $this->AddNotification = new AddNotification($message_data, 
                    $memreas_tables, $service_locator);
        }
    ...
    public function exec ()
    {
        ...
        try {
            ...
            if (isset($_POST['xml']) && ! empty($_POST['xml'])) {
                error_log(
                        "AddMediaEvent _POST ['xml'] ----> " . $_POST['xml'] .
                                 PHP_EOL);
                ...
                if (isset($data->addmediaevent->device_id)) {
                    $device_id = addslashes(
                            trim($data->addmediaevent->device_id));
                } else {
                    throw new \Exception('Error : device id is empty');
                }
                if (isset($data->addmediaevent->device_type)) {
                    $device_type = addslashes(
                            trim($data->addmediaevent->device_type));
                } else {
                    throw new \Exception('Error : device type is empty');
                }
                $event_id = isset($data->addmediaevent->event_id) ? addslashes(
                        trim($data->addmediaevent->event_id)) : '';
                $media_id = isset($data->addmediaevent->media_id) ? addslashes(
                        trim($data->addmediaevent->media_id)) : '';
                $is_profile_pic = isset($data->addmediaevent->is_profile_pic) ? addslashes(
                        trim($data->addmediaevent->is_profile_pic)) : 0;
                $is_server_image = addslashes(
                        trim($data->addmediaevent->is_server_image));
                $content_type = addslashes(
                        trim($data->addmediaevent->content_type));
                $s3url = addslashes(trim($data->addmediaevent->s3url));
                $s3file_name = addslashes(
                        trim($data->addmediaevent->s3file_name));
                ...
                $event_id = (isset($_POST['event_id'])) ? trim(
                        $_POST['event_id']) : '';
                $media_id = (isset($_POST['media_id'])) ? trim(
                        $_POST['media_id']) : '';
                $is_profile_pic = isset($_POST['is_profile_pic']) ? trim(
                        $_POST['is_profile_pic']) : 0;
            ...
            if ($is_server_image === 1) {
                ...
                // ///////////////////////////////////////
                $json_array = array();
                $s3file = (isset($_POST['s3file_name']) || isset($s3file_name)) ? $s3path .
                         $s3file_name : $s3url;
                ...
                $json_array['S3_files']['bucket'] = S3BUCKET;
                $json_array['S3_files']['location'] = json_decode(
                        (string) $location);
                ...
                if (strtolower($file_type[0]) == "video") {
                    ...
                    if (strtolower($file_type[0]) == "audio") {
                        $is_audio = 1;
                ...
                if (! empty($copyright)) {
                    ...
                    $copyright_batch = $this->dbAdapter->find(
                            'Application\Entity\CopyrightBatch', 
                            $copyright_batch_id);
                    if ($copyright_batch) {
                        //
                        // update used and add md5 and sha file checksums
                        //
                        $copyright_batch_json = $copyright_batch->__get(
                                'metadata');
                        $copyright_batch_array = json_decode(
                                $copyright_batch_json, true);
                        for ($i = 0; $i < count($copyright_batch_array); $i ++) {
                            if ($copyright_array['copyright_id'] ==
                                     $copyright_batch_array[$i]['copyright_id']) {
                                //
                                // update the entry
                                //
                        ...
                        //
                        Mlog::addone(' json_encode($copyright_batch_array) :: ', 
                                json_encode($copyright_batch_array));
                        $copyright_batch->__set('metadata', 
                                json_encode($copyright_batch_array));
                ...
                $this->dbAdapter->flush();
                Mlog::addone(
                        'flushed to db to update media, copyright_batch, and copyright', 
                        '');
                ...
                if ($is_profile_pic === 1) {
                    ...
                    $remove_result = $this->dbAdapter->createQuery(
                            $remove_old_profile);
                ...
                if (isset($event_id) && ! empty($event_id)) {
                    ...
                    if (! is_audio) {
                        ...
                        $qb->join('Application\Entity\Friend', 'f', 'WITH', 
                                'ef.friend_id = f.friend_id AND ef.user_approve=1');
                        $qb->where('ef.event_id = ?1 AND ef.friend_id != ?2');
                        $qb->setParameter(1, $event_id);
                        $qb->setParameter(2, $user_id);
                        
                        $efusers = $qb->getQuery()->getResult();
                        $userOBj = $this->dbAdapter->find(
                                'Application\Entity\User', $user_id);
                        $eventRepo = $this->dbAdapter->getRepository(
                                'Application\Entity\Event');
                        
                        $eventOBj = $eventRepo->findOneBy(
                                array(
                                        'event_id' => $event_id
                                ));
                        $nmessage = $userOBj->username . ' Added Media to  ' .
                                 $eventOBj->name . ' event';
                        ...
                        foreach ($efusers as $ef) {
                            ...
                            $data['addNotification']['meta'] = json_encode(
                                    $meta);
                            Mlog::add(
                ...
                if (empty($is_server_image)) {
                    ...
                    $response = $aws_manager->snsProcessMediaPublish(
                            $message_data);
